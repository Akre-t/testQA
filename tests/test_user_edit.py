...
class TestUserEdit(BaseCase):
    ...
    def test_edit_just_created_user_by_other_authorizated_user(self):
        ...
        Assertions.assert_response_text(response4,"Токен не предоставлен")
        ...
        Assertions.assert_json_value_by_name(
            ...
            "firstName",
            user1_first_name,
            "Неверное имя пользователя после неправильного редактирования. Поле 'firstName' нельзя редактировать"
        )
    ...
    def test_edit_just_created_user_with_incorrect_data(self,incorrect_parameter):
        ...
        if incorrect_parameter == "email":
            Assertions.assert_response_text(response3,"Неправильный формат Почты")
        elif incorrect_parameter == "firstName":
            Assertions.assert_json_value_by_name(
                response3,
                "error",
                "Слишком короткое значение поля 'firstName'",
                "Неверное сообщение об ошибке при короткой длине параметра ответа 'firstName'")
        ...
        Assertions.assert_json_value_by_name(
            ...
            "firstName",
            first_name,
            f"'firstName' параметр не должен быть изменен после неправильного запроса на редактирование"
            f"Текущее значение: {first_name} "
        )
        Assertions.assert_json_value_by_name(
            response4,
            "email",
            email,
            f"Параметр 'email' не должен изменяться после некорректного запроса на редактирование"
            f"Текущее значение: {email} "
        )