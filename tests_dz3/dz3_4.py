...
class TestDz3_4:
    ...
    def test_check_user_agent(self,user_agent,platform,browser,device):
        ...
        assert platform == actual_platform, f"Параметр ответа 'platform' некорректный: {actual_platform}. Ожидалось: {platform}"
        assert browser == actual_browser,f"Параметр ответа 'browser' are NOT некорректный: {actual_browser}. Ожидалось: {browser}"
        assert device == actual_device, f"Параметр ответа 'browser' are NOT некорректный: {actual_device}. Ожидалось: {device}"